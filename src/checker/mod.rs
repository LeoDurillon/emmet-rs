...
mod checker_test {
  ...
  fn should_check_structure() {
    ...
    assert_eq!(
      true,
      input_correctly_structured("(html>div>(p+div>p))+icon")
    );

    assert_eq!(
      true,
      input_correctly_structured("(html.test.class>div:test:prop>(p+div>p))*3+icon>p")
    );
    ...
    assert_eq!(false, input_correctly_structured("div+("));

    assert_eq!(true, input_correctly_structured("div+(div>p)"));
